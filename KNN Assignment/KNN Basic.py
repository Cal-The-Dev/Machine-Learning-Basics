...
def cluster_plots(dataset, medoidInd=[], colours = 'gray' , title = 'Dataset'):
    ...
    ax.scatter(dataset[:, 0], dataset[:, 1],s=8,lw=1,c= 'red')

    #Plot means if they are given
    if len(medoidInd) > 0:
        ax.scatter(dataset[medoidInd, 0], dataset[medoidInd, 1],s=8,lw=6,c='darkRed')
    fig.tight_layout()
    plt.show()
...
def kMeans(k, dataset, clusters, maxIter=10):
    ...
    print('Initial objective function: %.2f' % bestObjective, ';    Initial medoid indices: ', meansInd)
...
for i in range(Maxk):
    kMeans(i+1,dataset,clusters)

Accuracy(Maxk, BCubed, Recall, "BCubed, Recall and F-Score over k clusters")