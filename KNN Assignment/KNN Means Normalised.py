...
#Given dataset and indices of Medians, the function updates the clusters of the objects in the dataset
def assign(MediansInd, dataset, clusters):
...
#Compute the objective fuction for the given dataset and the set of Medians
def objectiveFunc(MediansInd, dataset):
    numOfObjects = len(dataset)
    
    clusters = np.zeros((len(dataset),2))
    
    #assign objects to closest Medians
    assign(MediansInd, dataset, clusters)
    
    #compute the objective function: the total sum of distances from objects to their Medians
    obj = 0
    for i in range(numOfObjects):
...
def kMedians(k, dataset, clusters, maxIter=10):
    ...
    #Generate indices for initial Medians
    np.random.seed(45)
    MediansInd = np.random.choice(numOfObjects, k, replace=False)
    
    #normalise dataset    
    for i in range(len(dataset)):
        #calculate length of each vector
        norm = np.sqrt(np.sum(np.square(dataset[i,:])))
        #divide all elements of vector by this length to give a new vector with normalised length 1
        dataset[i] = np.divide(dataset[i],norm)
    
    #check for length of vector after normalisation
    #print(np.sqrt(np.sum(np.square(dataset[1,:]))))
        
    #Make initial assignment of objects to the clusters
    assign(MediansInd, dataset,clusters)
    
    #plot initial assignment
    cluster_plots(dataset, MediansInd, clusters, title='Initial cluster assignment')
    ...
    tempMediansInd = np.copy(MediansInd)
    bestObjective = objectiveFunc(tempMediansInd, dataset)
    print('Initial objective function: %.2f' % bestObjective, ';    Initial Median indices: ', MediansInd)
    ...
    for i in range(maxIter):
        ...
        for y in range(k):
            for x in range(numOfObjects):
                if not x in tempMediansInd:
                    #put X into the set of Medians and instead of Y
                    tempMediansInd[y] = x
                    #and compute the value of the objective function for the new set of Medians
                    tempObjective = objectiveFunc(tempMediansInd, dataset)
                    ...
                    tempMediansInd[y] = MediansInd[y]
                                            
        #If the objectvie function has improved in the current iteration, then
        if isObjectiveImproved:
            #update the set of Medians according to the best pair (X,Y)
            MediansInd[int(bestPair[1])] = bestPair[0];
            #reassign objects to new Medians
            assign(MediansInd, dataset, clusters)
            bestObjective = objectiveFunc(MediansInd, dataset)
            #and plot the current clustering           
            cluster_plots(dataset, MediansInd, clusters, title='Clustering improvement')
            print('Objective function: %.2f' % bestObjective, ';    Median indices: ', MediansInd)
        else:
...
for i in range(Maxk):
    kMedians(i+1,data,clusters)

Accuracy(Maxk, BCubed, Recall, "BCubed, Recall and F-Score over k clusters using K-Medians CLustering")